...
class BlockDeviceMapping(base.NovaPersistentObject, base.NovaObject,
    ...
    def get_by_instance_and_volume_id(cls, context, volume_id, instance_uuid,
                                      expected_attrs=None):
        db_bdm = db.block_device_mapping_get_all_by_instance_and_volume_id(
            context, volume_id, instance_uuid)
        if not db_bdm:
            raise exception.VolumeBDMForInstanceNotFound(
                volume_id=volume_id, instance_uuid=instance_uuid)
        if len(db_bdm) > 1:
            raise exception.MultipleVolumeBDMForInstanceFound(
                volume_id=volume_id, instance_uuid=instance_uuid)
        db_bdm = db_bdm[0]
        return cls._from_db_object(context, cls(), db_bdm,
                                   expected_attrs=expected_attrs)
