...
class API(object):
    ...
    def check_attach(self, context, volume, instance=None):
        ...
        if 'multiattach' in volume and bool(volume['multiattach']):
    ...
    def get_volume_attachment(self, volume, instance_uuid):
        ...
        for attach in attachments:
            if attach['server_id'] == instance_uuid:
                return attach
        return None

    @translate_volume_exception
    def detach(self, context, volume_id, attachment_uuid):
            ...
            with excutils.save_and_reraise_exception():
                LOG.error(_LE('Initialize connection failed for volume '
                          ...
                          {'vol': volume_id,
                           ...
                           'code': ex.code})
                try:
                    self.check_and_terminate_connection(context, volume_id,
                                            connector, connector.get('host'))
    ...
    @translate_volume_exception
    def check_and_terminate_connection(self, context, volume_id, connector,
                                       host):
        instances = objects.InstanceList.get_by_host(context, host)
        vol = self.get(context, volume_id)
        attachments = vol['attachments']

        # attachments is a list of dicts
        attached_instance_ids = []
        for a in attachments:
            attached_instance_ids.append(a['server_id'])

        cross_list = [inst.uuid for inst in instances
                      if inst.uuid in attached_instance_ids]
        if len(cross_list) > 1:
            return

        return self.terminate_connection(context, volume_id, connector)
