...
def _populate_instance_types(instance_types_table):
    default_inst_types = {}
    vcpu_mem = {1 : (512, 1 * 1024),
               2 : (512, 1 * 1024, 2 * 1024, 4 * 1024, 8 * 1024),
               4 : (2 * 1024, 4 * 1024, 8 * 1024, 16 * 1024),
               8 : (4 * 1024, 8 * 1024, 16 * 1024, 32 * 1024, 64 * 1024),
               16 : (8 * 1024, 16 * 1024, 32 * 1024, 64 * 1024),
               32 : (8 * 1024, 16 * 1024, 32 * 1024, 64 * 1024, 96 * 1024),
               64 : (8 * 1024, 16 * 1024, 32 * 1024, 64 * 1024, 96 * 1024)}
    disks = (20, 40, 60, 80, 100, 200, 500)
    flavid = 1
    for vcpu in sorted(vcpu_mem.keys()):
        for mem in vcpu_mem[vcpu]:
            for disk in disks:
                default_inst_types[str(vcpu) + "-" + str(mem) + "-" + str(disk)] = \
                dict(mem=mem, vcpus=vcpu, root_gb=disk, eph_gb=0, flavid=flavid)
                flavid += 1
