...
class API(base.Base):
    ...
    def _check_and_transform_bdm(self, context, base_options, instance_type,
        ...
        # NOTE (jahei): If multi-attach is supported. It is possible to
        # to attach a volume to all the instances in this batch creating.
        ...
        if block_device_mapping:
            for bdm in block_device_mapping:
                if (bdm.get('boot_index') == 0 and
                            bdm.get('source_type') == 'volume' and
                            bdm.get('destination_type') == 'volume'):

                    volume_id = bdm.get('volume_id')
                    volume = self.volume_api.get(context, volume_id)
                    if volume and volume['multiattach']:
                        raise exception.BootFromMultiattachVolumeNotSupported(
                            volume_id=volume_id)
    ...
    def _local_delete(self, context, instance, bdms, delete_type, cb):
        ...
        for bdm in bdms:
            if bdm.is_volume:
                ...
                try:
                    self.volume_api.check_and_terminate_connection(context,
                        bdm.volume_id, connector, instance.host)
    ...
    def _detach_volume(self, context, instance, volume):
        ...
        try:
            ...
            LOG.warning(_LW("Failed to set the volume state to detaching. "
                            "Proceed anyway."),
                        instance=instance)
            # If previous detach failed, this step will raise
            # exception and prevent further action.
            pass
    ...
    def detach_volume(self, context, instance, volume):
        ...
        if volume['attachments']:
            for attachment in volume['attachments']:
                if attachment['server_id'] == instance['uuid']:
                    break
            else:
