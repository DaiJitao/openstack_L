...
class VMwareVMOps(object):
    ...
    def build_virtual_machine(self, instance, image_info,
        ...
        if instance.get('metadata'):
            esxi_host = instance.metadata.get('host')
            if esxi_host:
                host_mor = vm_util.get_host_ref_by_name(self._session,
                                                        self._cluster,
                                                        esxi_host)
        sriov_flag = False
        for vif_info in vif_infos:
            if vif_info['vif_model'] == 'sriov':
                sriov_flag = True

        cpu_aff_flag = False
        if instance.get('metadata') and instance.metadata.get('cpuAffinity') == 'True':
            cpu_aff_flag = True

        if not host_mor:
            if sriov_flag and cpu_aff_flag:
                host_mor = vm_util.get_host_by_sriov_and_cpuAff(self._session, self._cluster, instance)
            elif sriov_flag:
                host_mor = vm_util.get_host_by_sriov(self._session, self._cluster)
            elif cpu_aff_flag:
                host_mor = vm_util.get_host_by_cpuAff(self._session, self._cluster, instance)
    ...
    def attach_interface(self, instance, image_meta, vif):
        ...
        if vif['vnic_type'] == 'direct':
            vif_model = 'sriov'
        else:
            vif_model = image_meta.properties.get('hw_vif_model',
                                              constants.DEFAULT_VIF_MODEL)
        vm_state = vm_util.get_vm_state(self._session, instance)
        if vif_model == 'sriov' and vm_state != 'poweredOff':
            LOG.error("please stop the instance when attach sriov interface")
        ...
        with lockutils.lock(instance.uuid,
            ...
            client_factory = self._session.vim.client.factory
            attach_config_spec = vm_util.get_network_attach_config_spec(self._session, self._cluster,
                                        client_factory, vif_info, port_index, instance)
    ...
    def detach_interface(self, instance, vif):
        # this code is for  sync openstack db, If interface is detached
        # by vmware, don't take action
        meta_key = "sync_vmware_os-interface"
        if meta_key in instance.metadata:
            LOG.debug("Detached interface: sync_vmware_os-interface")
            return

        """Detach an interface from the instance."""
        vm_state = vm_util.get_vm_state(self._session, instance)
        if vif['vnic_type'] == 'direct' and vm_state != 'poweredOff':
            LOG.error("please stop the instance when detach sriov interface")
            raise exception.InterfaceAttachFailed(
                        instance_uuid=instance.uuid)
        vm_ref = vm_util.get_vm_ref(self._session, instance)
        # Ensure that there is not a race with the port index management
        with lockutils.lock(instance.uuid,
            ...
            detach_config_spec = vm_util.get_network_detach_config_spec(
                                        instance, client_factory, device, port_index)
