...
def get_vm_create_spec(client_factory, instance, data_store_name,
                       ...
                       profile_spec=None, metadata=None, host=None,
                       session=None, cluster=None):
    ...
    for vif_info in vif_infos:
        vif_spec = _create_vif_spec(client_factory, vif_info,
                                    instance, host, session,
                                    cluster)
        devices.append(vif_spec)

    # config reservation resources for memory
    for vif_info in vif_infos:
        if vif_info['vif_model'] == 'VirtualSriovEthernetCard':
            MemResAllInfo = client_factory.create('ns0:ResourceAllocationInfo')
            MemResAllInfo.reservation = int(instance.memory_mb)
            config_spec.memoryAllocation = MemResAllInfo
            break
    ...
    if instance.metadata.get("cpuAffinity") and instance.metadata.get("cpuAffinity") != 'True':
        cpus_set_str = instance.metadata.get("cpuAffinity")
        config_spec.cpuAffinity.affinitySet = [int(x) for x in cpus_set_str.split(',')]
    elif instance.metadata.get("cpuAffinity") == 'True':
        cpu_set_aviliable = get_cpus_set(session, host)
        cpu_choiced_set = random.sample(cpu_set_aviliable, instance.vcpus)
        config_spec.cpuAffinity.affinitySet = cpu_choiced_set
        LOG.info('cpu affinity is %(cpu_aff_set)s,'
                  ' random_checked_cpu_set is %(cpu_choice_set)s'
                 % {'cpu_aff_set': cpu_set_aviliable, 'cpu_choice_set': cpu_choiced_set})

    if instance.metadata.get("numa.nodeAffinity"):
        nodes_set = instance.metadata.get("numa.nodeAffinity")
        opt = client_factory.create('ns0:OptionValue')
        opt.key = 'numa.nodeAffinity'
        opt.value = nodes_set
        extra_config.append(opt)
    if instance.metadata.get("cpuid.coresPerSocket"):
        cores_per_socket = int(instance.metadata.get("cpuid.coresPerSocket"))
        if int(instance.vcpus) % cores_per_socket != 0:
            msg = _('instance cpu num is not divisible by '
                    'cpuid.coresPerSocket')
            raise exception.InvalidMetadata(reason=msg)
        opt0 = client_factory.create('ns0:OptionValue')
        opt0.key = 'numa.vcpu.followcorespersocke'
        opt0.value = 1
        extra_config.append(opt0)
        opt1 = client_factory.create('ns0:OptionValue')
        opt1.key = 'numa.vcpu.min'
        opt1.value = 1
        extra_config.append(opt1)
        opt2 = client_factory.create('ns0:OptionValue')
        opt2.key = 'cpuid.coresPerSocket'
        opt2.value = int(instance.metadata.get("cpuid.coresPerSocket"))
        extra_config.append(opt2)
...
def get_vm_resize_spec(client_factory, vcpus, memory_mb, extra_specs,
                       metadata=None,  instance_meta=None):
    ...
    if instance_meta.get('PF_DeviceId', None):
        MemResAllInfo = client_factory.create('ns0:ResourceAllocationInfo')
        MemResAllInfo.reservation = int(memory_mb)
        resize_spec.memoryAllocation = MemResAllInfo
...
def _create_vif_spec(client_factory, vif_info, instance=None, host=None,
                     session=None, cluster=None):
    """Builds a config spec for the addition of a new network
    adapter to the VM.
    """
    LOG.debug('vm_utils create_vif_spec instance is: %(instance)s  type is: %(type)s',
    ...
    #config Physical Function info. to the VirtualSriovEthernetCard deivce in vif_pec
    #Physical Function info include deivceId, id, systemiId, venderId, deviceName
    if vif_info['vif_model'] == 'VirtualSriovEthernetCard':
        if instance.metadata.get('PF_DeviceId', None) is not None:
            PFBacking = client_factory.create('ns0:VirtualPCIPassthroughDeviceBackingInfo')
            deviceId = instance.metadata.get('PF_DeviceId', None)
            if int(deviceId, 16) > 32767:
                PFBacking.deviceId = str(-int(bin((int(deviceId, 16) ^ 0xffff)+1), 2))
            else:
                PFBacking.deviceId = deviceId
            PFBacking.id = instance.metadata.get('PFId', None)
            PFBacking.systemId = instance.uuid
            vendorId = instance.metadata.get('vendorId', None)
            if int(vendorId, 16) > 32767:
                PFBacking.vendorId = -int(bin((int(vendorId, 16) ^ 0xffff)+1), 2)
            else:
                PFBacking.vendorId = vendorId
            PFBacking.deviceName = instance.metadata.get('DeviceName', None)
            SriovBacking = client_factory.create('ns0:VirtualSriovEthernetCardSriovBackingInfo')
            SriovBacking.physicalFunctionBacking = PFBacking
            net_device.sriovBacking = SriovBacking
        else:
            if not host:
                host = get_host_ref_for_vm(session, instance)
            pf_info = get_pf_by_host(session, host)
            PFBacking = client_factory.create('ns0:VirtualPCIPassthroughDeviceBackingInfo')
            PFBacking.deviceId = pf_info.deviceId
            #todo vctener 6.5 id value incloud Segment.bus:slot.function,but 5.5 is only need bus:slot.function
            PFBacking.id = pf_info.id
            PFBacking.systemId = instance.uuid
            PFBacking.vendorId = pf_info.vendorId
            PFBacking.deviceName = pf_info.deviceName
            SriovBacking = client_factory.create('ns0:VirtualSriovEthernetCardSriovBackingInfo')
            SriovBacking.physicalFunctionBacking = PFBacking
            net_device.sriovBacking = SriovBacking
...
def get_network_attach_config_spec(session, cluster, client_factory, vif_info, index, instance=None):
    ...
    #  config reservation resources for memory
    if vif_info['vif_model'] == 'VirtualSriovEthernetCard' and instance is not None:
        MemResAllInfo = client_factory.create('ns0:ResourceAllocationInfo')
        MemResAllInfo.reservation = int(instance.memory_mb)
        config_spec.memoryAllocation = MemResAllInfo
...
def get_network_detach_config_spec(instance, client_factory, device, port_index):
    ...
    if meta_key not in instance.metadata:
        ...
        if hasattr(device, 'sriovBacking'):
            device.sriovBacking.physicalFunctionBacking.deviceName = "BYPASS"
            device.sriovBacking.virtualFunctionBacking.deviceName = "BYPASS"
...
def get_root_disk_filename_for_sync_vmware(session, vm_ref):
    prop = 'config.extraConfig["root_disk"]'
    opt_val = session._call_method(vutil, 'get_object_property', vm_ref, prop)
...
def get_vmdk_info(session, vm_ref, uuid=None):
    ...
    root_disk = None
    root_disk_filename_sync_vmware = None
    root_device = None
    if uuid:
        root_disk = '%s.vmdk' % uuid
        root_disk_filename_sync_vmware = \
            get_root_disk_filename_for_sync_vmware(session, vm_ref)
    vmdk_device = None
        ...
        if device.__class__.__name__ == "VirtualDisk":
            if device.backing.__class__.__name__ == \
                ...
                if root_disk and path.basename == root_disk:
                    root_device = device
                elif root_disk and root_disk_filename_sync_vmware == \
...
def get_host_ref_by_name(session, cluster, host):
    """Get reference to a host within the cluster and host specified."""
    host_ret = session._call_method(vutil, "get_object_property",
                                    cluster, "host")
    if not host_ret or not host_ret.ManagedObjectReference:
        msg = _('No host available on cluster')
    ...
    for host_mor in host_ret.ManagedObjectReference:
        host_name = session._call_method(vutil,
                                         "get_object_property",
                                         host_mor,
                                         "summary.config.name")
        if host_name == host:
            LOG.debug('Found the host %s on cluster' % host)
            return host_mor
    else:
        msg = _('Not found the host %s on cluster' % host)
        raise exception.NoValidHost(reason=msg)


def get_host_by_sriov_and_cpuAff(session, cluster, instance):
    """Get reference to a host within the cluster by both sriov and cpu affinity."""
    host_ret = session._call_method(vutil,
                                    "get_object_property",
                                    cluster,
                                    "host")
    if not host_ret or not host_ret.ManagedObjectReference:
    ...
    host_mor_sriov = []
    for host_mor in host_ret.ManagedObjectReference:
        sriov_dev = session._call_method(vutil,
                                         "get_object_property",
                                         host_mor,
                                         "config.sriovDevicePool")
        cpu_enough = True
        if instance.get('metadata') and \
                        instance.metadata.get('cpuAffinity') == True:
            cpu_affinity_aviliable_set = get_cpus_set(session, host_mor)
            if len(cpu_affinity_aviliable_set) < int(instance.vcpus):
                cpu_enough = False
        if len(sriov_dev) > 0 and cpu_enough:
            host_mor_sriov.append(host_mor)
    if len(host_mor_sriov) > 0:
        host_mor_selected = host_mor_sriov[0]
        host_vm_num = session._call_method(vutil,
                                        "get_object_property",
                                        host_mor_sriov[0],
                                        "vm")
        vm_num_base = len(host_vm_num.ManagedObjectReference)

        for host_mor in host_mor_sriov:
            host_vms = session._call_method(vutil,
                                        "get_object_property",
                                        host_mor,
                                        "vm")
            if len(host_vms.ManagedObjectReference) < vm_num_base:
                vm_num_base = len(host_vms.ManagedObjectReference)
                host_mor_selected = host_mor
        LOG.debug(" checked host by sriov and cpu affinity "
                  "is %s" % host_mor_selected)
        return host_mor_selected
    else:
        msg = _('Not found hosts with sriov device or cpu '
                'affinity set not enough on cluster')
        raise exception.NoValidHost(reason=msg)


def get_host_by_sriov(session, cluster):
    """Get reference to a host within the cluster by sriov"""
    host_ret = session._call_method(vutil,
                                    "get_object_property",
                                    cluster,
                                    "host")
    ...
    if len(host_mor_sriov) > 0:
        ...
        for host_mor in host_mor_sriov:
            ...
            if len(host_vms.ManagedObjectReference) < vm_num_base:
                vm_num_base = len(host_vms.ManagedObjectReference)
                host_mor_selected = host_mor
        LOG.debug("checked host by only sriov "
                  "is %s" % host_mor_selected)
        return host_mor_selected
    else:
        msg = _('Not found hosts with sriov device or cpu affinity'
                ' set not enough on cluster')
        raise exception.NoValidHost(reason=msg)


def get_host_by_cpuAff(session, cluster, instance):
    """Get reference to a host within the cluster by cpu affinity"""
    host_ret = session._call_method(vutil,
                                    "get_object_property",
                                    cluster,
                                    "host")
    ...
    for host_mor in host_ret.ManagedObjectReference:
        ...
        if len(cpu_affinity_aviliable_set) < int(instance.vcpus):
            cpu_enough = False
        if cpu_enough:
            host_mor_cpu.append(host_mor)
    if len(host_mor_cpu) > 0:
        host_mor_selected = host_mor_cpu[0]
        cpu_set_num_base = len(get_cpus_set(session, host_mor_selected))
        for host_mor in host_mor_cpu:
            cpu_set = get_cpus_set(session, host_mor)
            cpu_set_num = len(cpu_set)
            if cpu_set_num > cpu_set_num_base:
                cpu_set_num_base = cpu_set_num
                host_mor_selected = host_mor
        LOG.debug(' checked host by only cpu affinity is %(host)s,'
                  ' cpu_set is %(cpu_set)s' % {'host': host_mor_selected,
                                               'cpu_set': cpu_set})
        return host_mor_selected
    else:
        msg = _('Not found hosts with sriov device or cpu affinity '
...
def get_cpus_set(session, host):
    """Get cpu set which not been affinity by specify host"""
    cpu_affinity_used_set = []
    vms = session._call_method(vutil,
                                 "get_object_property",
                                 host,
                                 "vm")
    for vm in vms.ManagedObjectReference:
        vms_cpu_set = session._call_method(vutil,
                                 "get_object_property",
                                 vm,
                                 "config.cpuAffinity")
        if vms_cpu_set is not None:
            cpu_affinity_used_set.extend(vms_cpu_set[0])
    host_cpu_threads = session._call_method(vutil,
                                 "get_object_property",
                                 host,
                                 "hardware.cpuInfo.numCpuThreads")
    cpu_set = range(host_cpu_threads)
    cpu_set_aviliable = list(set(cpu_set).difference(set(cpu_affinity_used_set)))
    return cpu_set_aviliable


def get_pf_by_host(session, host):
    """get Physical Function info by host specified"""
    host_pfs = session._call_method(vutil,
                                         "get_object_property",
                                         host,
                                         "config.sriovDevicePool")
    host_pf_index = random.randint(0, len(host_pfs)-1)
    pci_id = host_pfs[host_pf_index][0].pnic[0].pci
    host_pcidevices = session._call_method(vutil,
                                         "get_object_property",
                                         host,
                                         "hardware.pciDevice")

    for host_pci in host_pcidevices.HostPciDevice:
        if host_pci.id == pci_id:
            LOG.debug('host_pic is %s' % host_pci)
            LOG.debug('PF checked is %(pci_id)s on host %(host)s' %
                      {'pci_id': pci_id, 'host': host})
            return host_pci
    else:
        msg = _('Not found the PF %s on host' % pci_id)
        raise exception.PciDeviceNotFoundById(reason=msg)
...
def create_vm(session, instance, vm_folder, config_spec, res_pool_ref, host=None):
    """Create VM on ESX host."""
    LOG.debug("Creating VM on the ESX host", instance=instance)
    vm_create_task = session._call_method(
        session.vim,
        "CreateVM_Task", vm_folder,
        config=config_spec, pool=res_pool_ref, host=host)
    try:
