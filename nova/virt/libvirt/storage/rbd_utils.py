...
class RBDDriver(object):
    ...
    def create_snap(self, volume, name, pool=None, protect=True):
        ...
        encoded_name = self._encode_str(name)
        with RBDVolumeProxy(self, str(volume), pool=pool) as vol:
            tpool.execute(vol.create_snap, encoded_name)
            if protect and not vol.is_protected_snap(encoded_name):
                tpool.execute(vol.protect_snap, encoded_name)
    ...
    def remove_snap(self, volume, name, ignore_errors=False, pool=None,
        ...
        encoded_name = self._encode_str(name)
        with RBDVolumeProxy(self, str(volume), pool=pool) as vol:
            if encoded_name in [snap.get('name') for snap in vol.list_snaps()]:
                if vol.is_protected_snap(encoded_name):
                    if force:
                        tpool.execute(vol.unprotect_snap, encoded_name)
                ...
                LOG.debug('removing snapshot(%(name)s) on rbd image(%(img)s)',
                          {'name': name, 'img': volume})
                tpool.execute(vol.remove_snap, encoded_name)
            elif not ignore_errors:
    ...
    def rollback_to_snap(self, volume, name):
        ...
        with RBDVolumeProxy(self, volume) as vol:
            if encoded_name in [snap.get('name') for snap in vol.list_snaps()]:
                ...
                tpool.execute(vol.rollback_to_snap, encoded_name)
            else:
