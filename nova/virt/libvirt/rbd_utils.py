...
class RBDDriver(object):
    ...
    def snapshot(self, pool, image, snap_name):
        LOG.debug('snapshotting %(pool)s/%(img)s to snapshot %(snap)s' %
                  dict(pool = pool, img = image, snap = snap_name))
        with RADOSClient(self, str(pool)) as client:
            with rbd.Image(client.ioctx, str(image)) as img:
                try:
                    img.create_snap(str(snap_name))
                    img.protect_snap(str(snap_name))
                except rbd.ImageNotFound:
                    LOG.warn(_LW('rbd creating snapshot of %(pools)/%(img)s '
                                 'failed'),
                             dict(img = image, pool = pool))
