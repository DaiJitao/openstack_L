...
class VolumeBDMForInstanceNotFound(NotFound):
    msg_fmt = _("No volume Block Device Mapping with id %(volume_id)s "
                "for instance %(instance_uuid)s.")


class MultipleVolumeBDMForInstanceFound(NovaException):
    msg_fmt = _("Multiple volume Block Device Mapping with id %(volume_id)s "
                "for instance %(instance_uuid)s found.")


class MultiattachNotSupported(Invalid):
    msg_fmt = _("Volume %(volume_id)s is already attached, "
                "multi-attach is not supported by the virt driver.")


class BootFromMultiattachVolumeNotSupported(Invalid):
    msg_fmt = _("Boot failed for volume %(volume_id)s. Boot from volume "
                "is not supported for multi-attach volumes.")
